// Mock email service - nodemailer integration available when installed
...
export class EmailService {
  ...
  async sendWelcomeEmail(user: User): Promise<void> {
    ...
    try {
      ...
      console.warn('Failed to send welcome email:', (error as Error).message);
    }
  }

  async sendMFAEnabledEmail(user: User): Promise<void> {
    const mailOptions = {
      from: this.config.from,
      to: user.email,
    ...
    try {
      ...
      console.warn(
        'Failed to send MFA enabled email:',
        (error as Error).message
      );
    }
  }
