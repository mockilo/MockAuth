...
export class MockTailService implements MockDataGenerator {
  private config: MockTailConfig;

  constructor(config: MockTailConfig) {
    this.config = config;
  }

  generate(type: string, count: number = 10): any[] {
    const mockData: any[] = [];
  ...
  generateForSchema(schema: any, count: number = 10): any[] {
  ...
  private generateUser(id: number): any {
    const firstNames = [
      'John',
      'Jane',
      'Mike',
  ...
  private generatePost(id: number): any {
    const titles = [
    ...
    return {
      ...
      category: ['tech', 'tutorial', 'news', 'guide'][
  ...
  private generateProduct(id: number): any {
    const names = [
      'MockAuth Pro',
    ...
    return {
      ...
      price,
      currency: 'USD',
      category: ['software', 'tool', 'service', 'platform'][
        Math.floor(Math.random() * 4)
      ],
  ...
  private generateOrder(id: number): any {
    const statuses = [
      'pending',
      'processing',
      'shipped',
  ...
  private generateOrderItems(orderId: number): any[] {
  ...
  private generateGeneric(type: string, id: number): any {
    return {
      id,
      name: `${type} ${id}`,
  ...
  async exportToFile(data: any[], filename: string): Promise<void> {
    ...
    const outputPath = path.join(
      this.config.outputPath || './mock-data',
      filename
    );
...
export function createMockTailService(config: MockTailConfig): MockTailService {
