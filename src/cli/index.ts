...
import { MockAuthConfig, DatabaseConfig } from '../types';
import { AdvancedMockAuthCLI } from './advanced';
import EnhancedMockAuthCLI from './enhanced-cli';
import SimpleEnhancedCLI from './simple-enhanced';
...
export class MockAuthCLI {
  ...
  private findClosestCommand(typo: string, validCommands: string[]): string[] {
  ...
  public showInvalidCommand(typo: string, validCommands: string[]): void {
  ...
  public showCommandHelp(command: string): void {
...
💡 Tip: Run mockauth <command> --help for detailed info about a specific command
  ...
  private async initProject(options: CLIOptions): Promise<void> {
  ...
  private async createInteractiveConfig(): Promise<MockAuthConfig> {
    ...
    if (port) config.port = parseInt(port) || 3001;

    const baseUrl = await this.question('Base URL (http://localhost:3001): ');
    if (baseUrl) config.baseUrl = baseUrl;

    const database = await this.question(
      'Database type (memory|sqlite|postgresql|mysql) [memory]: '
    );
    if (
      database &&
      ['memory', 'sqlite', 'postgresql', 'mysql'].includes(database)
    ) {
      config.database!.type = database as any;
    }

    const enableMFA = await this.question('Enable MFA? (y/n) [y]: ');
    config.enableMFA = enableMFA.toLowerCase() !== 'n';

    const enableEcosystem = await this.question(
      'Enable MockTail & SchemaGhost? (y/n) [y]: '
    );
    ...
    const createUsers = await this.question(
      'Create default users? (y/n) [y]: '
    );
  ...
  private createExampleFiles(config: MockAuthConfig): void {
...
startAdvancedMockAuth().catch(console.error);`;
    ...
    fs.writeFileSync(
      path.join(examplesDir, 'advanced-usage.js'),
      advancedExample
    );
...
testMockAuth().catch(console.error);`;
  ...
  private async startServer(options: CLIOptions): Promise<void> {
    ...
    try {
      ...
      console.error('❌ Error starting MockAuth:', (error as Error).message);
...
testMockAuth().catch(console.error);`;
  ...
  private async generateData(options: CLIOptions): Promise<void> {
    ...
    fs.writeFileSync(
      path.join(outputPath, 'posts.json'),
  ...
  private async migrateDatabase(options: CLIOptions): Promise<void> {
  ...
  private async launchBuilder(options: CLIOptions): Promise<void> {
  ...
  private async startDebugMode(options: CLIOptions): Promise<void> {
    ...
    try {
      ...
      console.error('❌ Error starting debug mode:', (error as Error).message);
      process.exit(1);
    }
  }
  ...
  private async runHealthCheck(options: CLIOptions): Promise<void> {
    ...
    try {
      ...
      console.log(
        `✅ Server: ${healthStatus.server ? 'Running' : 'Failed'} on port ${config.port}`
      );
      console.log(
        `✅ Database: ${healthStatus.database ? 'Connected' : 'Failed'}`
      );
      console.log(
        `✅ Memory: ${healthStatus.memory}MB (${healthStatus.memoryStatus})`
      );
      console.log(
        `✅ Response Time: ${healthStatus.responseTime}ms (${healthStatus.performanceStatus})`
      );
      ...
      if (healthStatus.suggestions.length > 0) {
        console.log('\n💡 Suggestions:');
        healthStatus.suggestions.forEach((suggestion: string) => {
  ...
  private async stopServer(options: CLIOptions): Promise<void> {
  ...
  private async restartServer(options: CLIOptions): Promise<void> {
    ...
    try {
      ...
      console.error('❌ Error restarting server:', (error as Error).message);
    }
  }

  private async resetServer(options: CLIOptions): Promise<void> {
    ...
    try {
      ...
      console.error('❌ Error resetting server:', (error as Error).message);
    }
  }

  private async checkServerStatus(options: CLIOptions): Promise<void> {
  ...
  private async listServers(options: CLIOptions): Promise<void> {
  ...
  private async killAllServers(options: CLIOptions): Promise<void> {
    ...
    try {
      ...
      for (const server of runningServers) {
        ...
        try {
          ...
          console.log(
            `⚠️  Could not stop server on port ${server.port}: ${(error as Error).message}`
          );
        }
      }
      ...
      console.error('❌ Error stopping servers:', (error as Error).message);
    }
  }

  private async generateMigration(options: CLIOptions): Promise<void> {
    ...
    console.log('\n🧪 Test your migration:');
    console.log(
      `   npx mockauth test-migration --from mockauth --to ${provider}`
    );
  }
  ...
  private async performHealthChecks(auth: MockAuth): Promise<any> {
    ...
    const memoryStatus =
      memoryMB > 100 ? 'High' : memoryMB > 50 ? 'Normal' : 'Low';
    ...
    if (memoryStatus === 'High') {
      ...
      suggestions.push(
        'Consider restarting the server or optimizing configuration'
      );
    }
  ...
  private async testServerResponse(auth: MockAuth): Promise<boolean> {
    try {
      // Simple health check
      return true;
    } catch (error) {
  ...
  private async testDatabaseConnection(auth: MockAuth): Promise<boolean> {
    try {
      // Test database connection
      return true;
    } catch (error) {
  ...
  private async getActiveSessions(auth: MockAuth): Promise<number> {
    try {
...
if (require.main === module) {
  ...
  const isAdvancedCommand =
    cleanArgs.length > 0 && advancedCommands.includes(cleanArgs[0]);
  ...
  if (useLegacy) {
    ...
    const cli = new AdvancedMockAuthCLI();
    cli
      .run()
      .then(() => {
        cli.close();
